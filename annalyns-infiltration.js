...
const canExecuteFastAttack = (knightState) => {
  return (knightState ? false : true);
};

const canSpy = (knightState, archerState, prisonerState) => {
  if (knightState || archerState || prisonerState) {
    return true;
  };
  return false;
};

const canSignalPrisoner = (archerState, prisonerState) => {
  if (prisonerState) {
    return (archerState ? false : true);
  }
  return false;
};

const canFreePrisoner = (knightState, archerState, prisonerState, petDogState) => {
  if (petDogState) {
    return (archerState ? false : true);
  } else {
    if (prisonerState && !knightState && !archerState) {
      return true;
    }
    return false;
  };
